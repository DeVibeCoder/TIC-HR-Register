...
// Find OverviewPage start and end
const startMarker = '\nfunction OverviewPage({'
const endMarker   = '\nfunction EmployeeFormModal('
const startIdx = c.indexOf(startMarker)
const endIdx   = c.indexOf(endMarker)
if (startIdx === -1 || endIdx === -1) {
  console.error('OverviewPage bounds not found', startIdx, endIdx)
  process.exit(1)
}
...
function OverviewPage({
  ...
  const leaveStepMap = useMemo(() => {
    const m: Record<string, number> = {}
    leaveRequests.forEach(r => { m[r.step] = (m[r.step] ?? 0) + 1 })
    return m
  }, [leaveRequests])
  ...
  const natCounts = useMemo(() => {
    ...
    employees.forEach(e => { n[e.nationality] = (n[e.nationality] ?? 0) + 1 })
  ...
  // ── Alert list: items needing attention ──────────────────────────────────
  const alerts: { id: string; color: string; bg: string; icon: string; title: string; body: string }[] = []
  if (outStock > 0)
    alerts.push({ id:'out-stock', color:'#dc2626', bg:'#fef2f2', icon:'📦', title:`${outStock} item${outStock > 1 ? 's' : ''} out of stock`, body:'Check inventory — reorder required.' })
  if (lowStock > 0)
    alerts.push({ id:'low-stock', color:'#d97706', bg:'#fffbeb', icon:'⚠️', title:`${lowStock} item${lowStock > 1 ? 's' : ''} running low`, body:'Stock below reorder level.' })
  if (urgentMed > 0)
    alerts.push({ id:'med-urgent', color:'#dc2626', bg:'#fef2f2', icon:'🚨', title:`${urgentMed} urgent medical case${urgentMed > 1 ? 's' : ''}`, body: admitted > 0 ? `${admitted} currently admitted.` : 'Requires follow-up.' })
  if (pending > 0)
    alerts.push({ id:'pending', color:'#7c3aed', bg:'#f5f3ff', icon:'📋', title:`${pending} employee record${pending > 1 ? 's' : ''} incomplete`, body:'Missing NIC, Work Permit or other details.' })
  if (passAtHO > 0)
    alerts.push({ id:'pass-ho', color:'#2563eb', bg:'#eff6ff', icon:'🛂', title:`${passAtHO} passport${passAtHO > 1 ? 's' : ''} pending HO confirmation`, body:'Awaiting received-by confirmation from HQ.' })
  if (noticeTerminations.length > 0)
    alerts.push({ id:'term', color:'#f59e0b', bg:'#fffbeb', icon:'📤', title:`${noticeTerminations.length} active termination${noticeTerminations.length > 1 ? 's' : ''}`, body:'Notices in progress — follow up required.' })
    ...
    <section className="dash-wrap">
      ...
      <div className="dash-grid">
        ...
        <div className="dash-col">
          ...
          {/* Terminations + Medical side by side */}
          ...
          {/* Record Completion + Exit Interviews side by side */}
          <div className="dash-row-pair">
...
c = c.slice(0, startIdx) + NEW_OVERVIEW + c.slice(endIdx)
